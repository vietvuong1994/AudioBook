  ...
  confirmDownloadNewBook = async bookData => {
    ...
    if (currentReadBookId !== downloadingBookData.id) {
      ...
      this.props.showModal(
        {
          open: true,
          message: "Không thể xoá cuốn sách đang được nghe",
          closeModal: this.closeModal
        },
        "alert"
      );
    ...
    if (connecting) {
      if (chapters.length > 0) {
        this.props.showModal(
          {
            open: true,
            message:
              "Bạn đang tải xuống 1 cuốn sách khác. \nĐể tải cuốn sách này cần bỏ cuốn sách đang tải xuống",
        ...
        this.props.showModal(
          {
            open: true,
            message: `Bạn đã tải xuống quá số lượng cho phép. \nĐể tải xuống sẽ bỏ cuốn sách ${removeBookName}`,
